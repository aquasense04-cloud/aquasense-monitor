...
app = Flask(__name__)
CORS(app)  # Allow cross-origin requests so any frontend can read the API
...
@app.route("/")
def serve_new_dashboard():
    """Serves the new AquaSense landing page (index.html next to this file)."""
...
def serve_old_dashboard():
    """Serves the original embedded dashboard at /monitor."""
    return Response(DASHBOARD_HTML, mimetype="text/html")

@app.route("/<path:filename>")
def serve_static(filename):
    """Serves static files (images, etc.) from the same directory as server.py."""
